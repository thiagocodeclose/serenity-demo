...
export function Garrison365LivePreview() {
    ...
    function handler(e: MessageEvent) {
        ...
        if (active) {
          ...
          requestAnimationFrame(() =>
            requestAnimationFrame(() => {
              ...
              // 1. From __GARRISON365_REGISTRY__ (useGarrison365Element hooks)
              if (registry) {
              ...
              window.parent?.postMessage(
                { type: "GARRISON365_MANIFEST", templateId, templateVersion, elements },
                "*",
              );
            }),
          );
      ...
      if (e.data.type === "GARRISON365_INLINE_EDIT_REQUEST") {
        ...
        requestAnimationFrame(() => {
          ...
          window.parent.postMessage({ type: "GARRISON365_INLINE_EDIT_START", payload: { id } }, "*");
        });
        return;
      }
  ...
  // and sends GARRISON365_INLINE_EDIT_REQUEST when it detects a double-click.
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (!el) {
        setOverlay(null);
        window.parent.postMessage({ type: "GARRISON365_ELEMENT_CLICK", payload: { id: null } }, "*");
        return;
      }
      ...
      refreshOverlay(id);
      window.parent.postMessage({ type: "GARRISON365_ELEMENT_CLICK", payload: { id, computedColor, rect: elRect } }, "*");
    };
