...
export function PricingSection() {
  ...
  const widgetConfig = (
    siteData as unknown as { widgetConfig?: Record<string, unknown> }
  )?.widgetConfig;
  const billingEnabled = !!widgetConfig?.billing_enabled;
  const conversionMode =
    (widgetConfig?.conversion_mode as string) || "lead_only";
  const slug = siteData?.gym?.slug || garrison365.gymSlug;

  const eyebrow = useGarrison365Element(
    "pricing_eyebrow",
    { content: "Membership", visible: true },
    { section: "Pricing", label: "Eyebrow", type: "eyebrow" },
  );
  const headline = useGarrison365Element(
    "pricing_headline",
    { content: "Your Journey Starts Here", visible: true },
    { section: "Pricing", label: "Headline", type: "text" },
  );
  const descEl = useGarrison365Element(
    "pricing_description",
    {
      content: "Simple, transparent pricing. No hidden fees. Cancel anytime.",
      visible: true,
    },
    { section: "Pricing", label: "Description", type: "text" },
  );

  useEffect(() => {
    const baseUrl =
      process.env.NEXT_PUBLIC_CODEGYM_URL || "https://app.codegyms.com";
    const gymSlug = slug || garrison365.gymSlug;
    if (!gymSlug) {
      setLoading(false);
      return;
    }
    ...
    >
      <div className="container-tight">
        ...
        <div className="text-center mb-16">
          <Reveal>
            <p {...eyebrow.editProps} className="eyebrow mb-4">
              {eyebrow.content}
            </p>
          </Reveal>
          <Reveal delay={0.1}>
                      ...
                      <div>
                        <h3
                          style={{
                            ...
                            marginBottom: plan.description ? "0.4rem" : 0,
                          }}
                        >
                          {plan.name}
                        </h3>
